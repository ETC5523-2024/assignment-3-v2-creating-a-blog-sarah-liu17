...
The data is sourced from TidyTuesday packgae on {github}[https://github.com/rfordatascience/tidytuesday/blob/master/data/2023/2023-10-03/readme.md].
...
#2: Making category a variable by pivot longer, and extract the top 5 most popular grants
...
## Data dictionary
- opportunity_id:	Integer ID for this opportunity.
...
- category: category classified by the function of grant opportunities

# Top 5 grant categories

```{r}
#| label: fig-topcat
#| fig-cap: top 5 grant categories

grants_long %>% group_by(category) %>% summarise(count = n()) %>% arrange(desc(count)) %>% head(5) %>% 
...
It’s clear from @fig-topcat that the health sector dominates U.S. government grant funding in 2024, with over 1,000 grants posted. This reflects a national focus on public health and research into health issues, aiming to improve overall well-being. Innovation in science, including computer science and engineering, also sees substantial investment, indicating the government's emphasis on technological progress. Education, environment, and income security also receive significant attention, reinforcing support for human capital development, environmental protection, and social welfare.
...
ggplot(top_est_fund, aes(x = reorder(opportunity_title, total_funding), y = total_funding, fill = category)) +
  ...
  labs(title = "Top 7 Most funded Grant Opportunities",
       x = "Grant Opportunity",
       y = "Total funding",
       caption = "Figure 2: Top 7 grants by total estimated program funding") +
  theme_minimal()
```
...
In @fig-awards, Transportation demonstrate a high difference that soars to around $300 million. This means if you're working on transportation-related projects, you're looking at some serious funding potential. Infrastructure projects, for example, often require large sums, and this category reflects that.
...
So, depending on your project’s needs, focusing on high-ceiling categories can give you a competitive edge when it comes to scaling, while smaller projects could thrive in categories offering more consistent funding. Knowing where the money is concentrated helps you strategize better and apply for the right grants!

## What's next?
By examining top funding categories and individual high-value grants, the report offers insights into where the US government allocates the most resources. The results indicate a substantial focus on health, environment, education, and infrastructure, with key opportunities for companies in these sectors to secure significant funding. The distribution of grant award ceilings and floors are also presented to guide companies in applying for grants that align with their financial needs.
...
Look at Award Sizes: Not every grant is going to meet your funding needs. Some grants are tailored for larger, well-established businesses, while others may be a better fit for startups or smaller projects.
...
Government grants play a pivotal role in distributing funding across key sectors of the U.S. economy. With billions of dollars allocated annually, it’s essential for businesses and organizations to identify where the government focuses its investments to make informed decisions on funding applications. This post explores the landscape of U.S. government grants in 2024, with a focus on identifying the top grant categories and the most well-funded individual opportunities.

The U.S. government allocates a substantial budget to support a variety of projects through grant programs, with a particular focus on health, education, environment, and infrastructure. Start-ups and organizations lacking sufficient cash flow, or those considered high risk by traditional funding sources, can benefit greatly from these grants.
...
```{r, fig.width=12, fig.height=8, out.width='100%'}
top_funding_award <- grant_detail_24 %>%
  group_by(opportunity_id) %>%
  summarise(total_no = sum(expected_number_of_awards, na.rm = TRUE)) %>%
  arrange(desc(total_no)) %>%
  head(5)

 top_funding_award <- top_funding_award %>%
  inner_join(grants, by = "opportunity_id") %>%
  select(opportunity_title, total_no) 
  ggplot(top_funding_award, aes(x = reorder(opportunity_title, total_no), y = total_no)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(title = "Top 5 Grant Opportunities by expected number of awards",
       x = "Grant Opportunity",
...
```{r}
grants_long %>% filter(category == "employment") %>% left_join(grant_detail_24, by = "opportunity_id") %>% group_by(funding_opportunity_title.x) %>% mutate(n = sum(estimated_total_program_funding.x)) %>% arrange(desc(n)) %>% head(10) %>% select(funding_opportunity_title.x, n)
```
